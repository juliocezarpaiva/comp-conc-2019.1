...
void multiplicaMatrizVetor(const float *a, const float *x, float *b, int linhas_inicio, int linhas_fim, int colunas) {
   int i, j;
   for (i=linhas_inicio; i<linhas_fim; i++) {
      b[i] = 0;
      for (j=0; j<colunas; j++) {
...
int main(int argc, char *argv[]) {
   double inicio, fim, delta1, delta2, delta3, delta4;
   float *matA; //matriz de entrada
   float *vetX; //vetor de entrada
   float *vetB; //vetor de saida
   ...
   */
    ...
    int nthreads; scanf("Numero de threads: %d\n", &nthreads);
    int *tid;
    int tam_bloco = linhas/nthreads;
    int inicio_bloco, fim_bloco;
    ...
    int indice = * (int *) tid;
    inicio_bloco = indice * tam_bloco;
    if (indice < nthreads-1) fim_bloco = inicio_bloco + tam_bloco; 
    else fim_bloco = tam_bloco;
    for(indice = inicio_bloco; indice < fim_bloco; indice++)
    {
        //vet[i]++;
        //multiplicaMatrizVetor(matA, vetX, vetB, linhas, colunas)
        //multiplicaMatrizVetor(const float *a, const float *x, float *b, int linhas_inicio, int linhas_fim, int colunas)
    }
    free(tid);
